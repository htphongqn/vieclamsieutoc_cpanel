    ...
    public partial class config_footer : System.Web.UI.Page
    {
        ...
        private void showFileHTML1()
        {
            try
            {
                string pathFile;
                string strHTMLContent;
                pathFile = Server.MapPath(PathFiles.GetPathFooter() + "/footer-EN.htm");
                if ((File.Exists(pathFile)))
                {
                    StreamReader objNewsReader;
                    //objNewsReader = New StreamReader(pathFile, System.Text.Encoding.Default)
        ...
        private void showFileHTML2()
        {
            try
            {
                string pathFile;
                string strHTMLContent;
                pathFile = Server.MapPath(PathFiles.GetPathFooter() + "/footer-ru.htm");
                if ((File.Exists(pathFile)))
                {
                    StreamReader objNewsReader;
                    //objNewsReader = New StreamReader(pathFile, System.Text.Encoding.Default)
        ...
        private void SaveHTMLInfo1()
        {
            try
            {
                ...
                strFileName = PathFiles.GetPathFooter() + "/footer-EN.htm";
                strHTMLFileLocation = Server.MapPath(strFileName);
        ...
        private void SaveHTMLInfo2()
        {
            try
            {
                ...
                strFileName = PathFiles.GetPathFooter() + "/footer-ru.htm";
                strHTMLFileLocation = Server.MapPath(strFileName);
