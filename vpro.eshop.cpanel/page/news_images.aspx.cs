...
namespace vpro.eshop.cpanel.page
{
    public partial class news_images : System.Web.UI.Page
    {
        ...
        private void getInfo()
        {
            try
            {
                ...
                if (G_info.ToList().Count > 0)
                {
                    ...
                    rblType.SelectedValue = Utils.CStrDef(G_info.ToList()[0].NEWS_IMG_TYPE);
                    //image 1
        ...
        public string getType(object obj_id)
        {
            int idType = Utils.CIntDef(obj_id);
            switch (idType)
            {
                case 1:
                    return "Giới thiệu";
        ...
        protected void Lnupload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int img_id = Utils.CIntDef(Request.QueryString["img_id"]);
                var img_update = DB.ESHOP_NEWS_IMAGEs.Where(n => n.NEWS_IMG_ID == img_id).ToList();
                ...
                for (int i = 0; i < hfc.Count; i++)
                {
                    ...
                    {
                        if (img_update != null && img_update.Count > 0)
                        {
                            var g_update = img_update[0];
                            string pathfile = Server.MapPath("/data/news/" + m_news_id);
                            ...
                            g_update.NEWS_IMG_TYPE = Utils.CIntDef(rblType.SelectedValue);
