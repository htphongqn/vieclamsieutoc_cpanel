...
{
    public partial class capbac : System.Web.UI.Page
    {
        #region Declare
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ucHeader.HeaderLevel1 = "DS cấp bậc";
                ucHeader.HeaderLevel1_Url = "../page/capbacs.aspx";
                ucHeader.HeaderLevel2 = "Thêm mới cấp bậc";
                ucHeader.HeaderLevel2_Url = "../page/capbac.aspx";
        ...
        private void getInfo()
        {
            try
            {
                var G_info = (from c in DB.VL_CAPBACs
                              where c.ID == _id
                              select c
                            );
        ...
        private void SaveInfo(string strLink = "")
        {
            try
            {
                ...
                if (_id == 0)
                {
                    ...
                    VL_CAPBAC insert = new VL_CAPBAC();
                    ...
                    DB.VL_CAPBACs.InsertOnSubmit(insert);
                    DB.SubmitChanges();

                    _id = Utils.CIntDef(insert.ID);

                    strLink = string.IsNullOrEmpty(strLink) ? "capbacs.aspx?id=" + _id : strLink;
                }
                else
                {
                    //update
                    var c_update = DB.GetTable<VL_CAPBAC>().Where(g => g.ID == _id);
                    ...
                    if (c_update.ToList().Count > 0)
                    {
                        ...
                        strLink = string.IsNullOrEmpty(strLink) ? "capbacs.aspx" : strLink;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DeleteInfo()
        {
            try
            {
                var G_info = DB.GetTable<VL_CAPBAC>().Where(g => g.ID == _id);

                DB.VL_CAPBACs.DeleteAllOnSubmit(G_info);
                DB.SubmitChanges();
                Response.Redirect("capbacs.aspx");
            }
            catch (Exception ex)
            {
                clsVproErrorHandler.HandlerError(ex);
        ...
        {
            return "capbac.aspx?id=" + Utils.CStrDef(GroupId);
        }
