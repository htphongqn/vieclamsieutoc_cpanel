...
{
    public partial class mucluong : System.Web.UI.Page
    {
        #region Declare
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ucHeader.HeaderLevel1 = "DS mức lương";
                ucHeader.HeaderLevel1_Url = "../page/mucluongs.aspx";
                ucHeader.HeaderLevel2 = "Thêm mới mức lương";
                ucHeader.HeaderLevel2_Url = "../page/mucluong.aspx";
        ...
        private void getInfo()
        {
            try
            {
                var G_info = (from c in DB.VL_MUCLUONGs
                              where c.ID == _id
                              select c
                            );
        ...
        private void SaveInfo(string strLink = "")
        {
            try
            {
                ...
                if (_id == 0)
                {
                    ...
                    VL_MUCLUONG insert = new VL_MUCLUONG();
                    ...
                    DB.VL_MUCLUONGs.InsertOnSubmit(insert);
                    DB.SubmitChanges();

                    _id = Utils.CIntDef(insert.ID);

                    strLink = string.IsNullOrEmpty(strLink) ? "mucluongs.aspx?id=" + _id : strLink;
                }
                else
                {
                    //update
                    var c_update = DB.GetTable<VL_MUCLUONG>().Where(g => g.ID == _id);
                    ...
                    if (c_update.ToList().Count > 0)
                    {
                        ...
                        strLink = string.IsNullOrEmpty(strLink) ? "mucluongs.aspx" : strLink;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DeleteInfo()
        {
            try
            {
                var G_info = DB.GetTable<VL_MUCLUONG>().Where(g => g.ID == _id);

                DB.VL_MUCLUONGs.DeleteAllOnSubmit(G_info);
                DB.SubmitChanges();
                Response.Redirect("mucluongs.aspx");
            }
            catch (Exception ex)
            {
                clsVproErrorHandler.HandlerError(ex);
        ...
        {
            return "mucluong.aspx?id=" + Utils.CStrDef(GroupId);
        }
