...
namespace vpro.eshop.cpanel.page
{
    public partial class khuvuc : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ucHeader.HeaderLevel1 = "DS khu vực";
                ucHeader.HeaderLevel1_Url = "../page/khuvucs.aspx";
                ucHeader.HeaderLevel2 = "Thêm mới khu vực";
                ucHeader.HeaderLevel2_Url = "../page/khuvuc.aspx";
        ...
        private void getInfo()
        {
            try
            {
                var G_info = (from c in DB.VL_AREAs
                              where c.ID == _id
                              select c
                            );
        ...
        private void SaveInfo(string strLink = "")
        {
            try
            {
                ...
                if (_id == 0)
                {
                    ...
                    DB.VL_AREAs.InsertOnSubmit(insert);
                    DB.SubmitChanges();

                    _id = Utils.CIntDef(insert.ID);

                    strLink = string.IsNullOrEmpty(strLink) ? "khuvucs.aspx?id=" + _id : strLink;
                }
                else
                {
                    //update
                    var c_update = DB.GetTable<VL_AREA>().Where(g => g.ID == _id);
                    ...
                    if (c_update.ToList().Count > 0)
                    {
                        ...
                        strLink = string.IsNullOrEmpty(strLink) ? "khuvucs.aspx" : strLink;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DeleteInfo()
        {
            try
            {
                var G_info = DB.GetTable<VL_AREA>().Where(g => g.ID == _id);

                DB.VL_AREAs.DeleteAllOnSubmit(G_info);
                DB.SubmitChanges();
                Response.Redirect("khuvucs.aspx");
            }
            catch (Exception ex)
            {
                clsVproErrorHandler.HandlerError(ex);
        ...
        {
            return "khuvuc.aspx?id=" + Utils.CStrDef(GroupId);
        }
