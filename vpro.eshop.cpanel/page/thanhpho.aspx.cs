...
namespace vpro.eshop.cpanel.page
{
    public partial class thanhpho : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ucHeader.HeaderLevel1 = "DS thành phố";
                ucHeader.HeaderLevel1_Url = "../page/thanhphos.aspx";
                ucHeader.HeaderLevel2 = "Thêm mới thành phố";
                ucHeader.HeaderLevel2_Url = "../page/thanhpho.aspx";
        ...
        private void getInfo()
        {
            try
            {
                var G_info = (from c in DB.VL_CITies
                              where c.ID == _id
                              select c
                            );
        ...
        private void SaveInfo(string strLink = "")
        {
            try
            {
                ...
                if (_id == 0)
                {
                    ...
                    DB.VL_CITies.InsertOnSubmit(insert);
                    DB.SubmitChanges();

                    _id = Utils.CIntDef(insert.ID);

                    strLink = string.IsNullOrEmpty(strLink) ? "thanhphos.aspx?id=" + _id : strLink;
                }
                else
                {
                    //update
                    var c_update = DB.GetTable<VL_CITY>().Where(g => g.ID == _id);
                    ...
                    if (c_update.ToList().Count > 0)
                    {
                        ...
                        strLink = string.IsNullOrEmpty(strLink) ? "thanhphos.aspx" : strLink;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DeleteInfo()
        {
            try
            {
                var G_info = DB.GetTable<VL_CITY>().Where(g => g.ID == _id);

                DB.VL_CITies.DeleteAllOnSubmit(G_info);
                DB.SubmitChanges();
                Response.Redirect("thanhphos.aspx");
            }
            catch (Exception ex)
            {
                clsVproErrorHandler.HandlerError(ex);
        ...
        {
            return "thanhpho.aspx?id=" + Utils.CStrDef(GroupId);
        }
