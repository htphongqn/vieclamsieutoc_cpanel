...
{
    public partial class hinhthucnophoso : System.Web.UI.Page
    {
        #region Declare
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ucHeader.HeaderLevel1 = "DS hình thức nộp hồ sơ";
                ucHeader.HeaderLevel1_Url = "../page/hinhthucnophosos.aspx";
                ucHeader.HeaderLevel2 = "Thêm mới hình thức nộp hồ sơ";
                ucHeader.HeaderLevel2_Url = "../page/hinhthucnophoso.aspx";
        ...
        private void getInfo()
        {
            try
            {
                var G_info = (from c in DB.VL_HINHTHUCNOPHOSOs
                              where c.ID == _id
                              select c
                            );
        ...
        private void SaveInfo(string strLink = "")
        {
            try
            {
                ...
                if (_id == 0)
                {
                    ...
                    VL_HINHTHUCNOPHOSO insert = new VL_HINHTHUCNOPHOSO();
                    ...
                    DB.VL_HINHTHUCNOPHOSOs.InsertOnSubmit(insert);
                    DB.SubmitChanges();

                    _id = Utils.CIntDef(insert.ID);

                    strLink = string.IsNullOrEmpty(strLink) ? "hinhthucnophosos.aspx?id=" + _id : strLink;
                }
                else
                {
                    //update
                    var c_update = DB.GetTable<VL_HINHTHUCNOPHOSO>().Where(g => g.ID == _id);
                    ...
                    if (c_update.ToList().Count > 0)
                    {
                        ...
                        strLink = string.IsNullOrEmpty(strLink) ? "hinhthucnophosos.aspx" : strLink;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DeleteInfo()
        {
            try
            {
                var G_info = DB.GetTable<VL_HINHTHUCNOPHOSO>().Where(g => g.ID == _id);

                DB.VL_HINHTHUCNOPHOSOs.DeleteAllOnSubmit(G_info);
                DB.SubmitChanges();
                Response.Redirect("hinhthucnophosos.aspx");
            }
            catch (Exception ex)
            {
                clsVproErrorHandler.HandlerError(ex);
        ...
        {
            return "hinhthucnophoso.aspx?id=" + Utils.CStrDef(GroupId);
        }
