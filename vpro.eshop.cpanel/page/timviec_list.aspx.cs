...
{
    public partial class timviec_list : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ucHeader.HeaderLevel2_Url = "../page/timviec_list.aspx";
                Loadchuyenmuc();
                SearchResult();
        ...
        {
            return "timviec.aspx?news_id=" + Utils.CStrDef(obj_id);
        }
        public string getLink_comment(object obj_id)
        {
            return "news_comment.aspx?news_id=" + Utils.CStrDef(obj_id);
        }
        public string getCustomerName(object customerId)
        {
            try
            {
                int _customerId = Utils.CIntDef(customerId);
                var _list = DB.GetTable<ESHOP_CUSTOMER>().Where(a => a.CUSTOMER_ID == _customerId);
                if (_list != null && _list.ToList().Count > 0)
                {
                    return _list.ToList()[0].CUSTOMER_FULLNAME;
                }
                else
                {
                    return "";
        ...
        public string getTypeNew(object obj_id)
        {
            ...
            {
                case 1: strResult = "Tìm việc";
                    break;
                case 2: strResult = "Tuyển dụng";
                    break;
                case 3: strResult = "Cẩm nang";
                    break;
                case 4: strResult = "Góp ý";
                    break;
        ...
        protected void lbtSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach (DataGridItem item in GridItemList.Items)
                {
                    ...
                    if (check.Checked)
                    {
                        strLink = "timviec.aspx?news_id=" + Utils.CStrDef(GridItemList.DataKeys[i]);
                        break;
                    }
                    i++;
                }
        ...
        protected void ddlTinhtranghoso_SelectedIndexChanged(object sender, EventArgs e)
        {
            SearchResult();
        }
    }
