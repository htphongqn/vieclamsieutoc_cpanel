...
namespace vpro.eshop.cpanel.page
{
    public partial class kinhnghiem : System.Web.UI.Page
    {
        ...
        private void getInfo()
        {
            try
            {
                var G_info = (from c in DB.VL_KINHNGHIEMs
                              where c.ID == _id
                              select c
                            );
        ...
        private void SaveInfo(string strLink = "")
        {
            try
            {
                ...
                if (_id == 0)
                {
                    ...
                    VL_KINHNGHIEM insert = new VL_KINHNGHIEM();
                    ...
                    DB.VL_KINHNGHIEMs.InsertOnSubmit(insert);
                    DB.SubmitChanges();

                    _id = Utils.CIntDef(insert.ID);

                    strLink = string.IsNullOrEmpty(strLink) ? "kinhnghiems.aspx?id=" + _id : strLink;
                }
                else
                {
                    //update
                    var c_update = DB.GetTable<VL_KINHNGHIEM>().Where(g => g.ID == _id);
                    ...
                    if (c_update.ToList().Count > 0)
                    {
                        ...
                        strLink = string.IsNullOrEmpty(strLink) ? "kinhnghiems.aspx" : strLink;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DeleteInfo()
        {
            try
            {
                var G_info = DB.GetTable<VL_KINHNGHIEM>().Where(g => g.ID == _id);

                DB.VL_KINHNGHIEMs.DeleteAllOnSubmit(G_info);
                DB.SubmitChanges();
        ...
        {
            return "kinhnghiem.aspx?id=" + Utils.CStrDef(GroupId);
        }
