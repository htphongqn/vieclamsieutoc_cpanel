...
{
    public partial class hinhthuclamviec : System.Web.UI.Page
    {
        #region Declare
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ucHeader.HeaderLevel1 = "DS hình thức làm việc";
                ucHeader.HeaderLevel1_Url = "../page/hinhthuclamviecs.aspx";
                ucHeader.HeaderLevel2 = "Thêm mới hình thức làm việc";
                ucHeader.HeaderLevel2_Url = "../page/hinhthuclamviec.aspx";
        ...
        private void getInfo()
        {
            try
            {
                var G_info = (from c in DB.VL_HINHTHUCLAMVIECs
                              where c.ID == _id
                              select c
                            );
        ...
        private void SaveInfo(string strLink = "")
        {
            try
            {
                ...
                if (_id == 0)
                {
                    ...
                    VL_HINHTHUCLAMVIEC insert = new VL_HINHTHUCLAMVIEC();
                    ...
                    DB.VL_HINHTHUCLAMVIECs.InsertOnSubmit(insert);
                    DB.SubmitChanges();

                    _id = Utils.CIntDef(insert.ID);

                    strLink = string.IsNullOrEmpty(strLink) ? "hinhthuclamviecs.aspx?id=" + _id : strLink;
                }
                else
                {
                    //update
                    var c_update = DB.GetTable<VL_HINHTHUCLAMVIEC>().Where(g => g.ID == _id);
                    ...
                    if (c_update.ToList().Count > 0)
                    {
                        ...
                        strLink = string.IsNullOrEmpty(strLink) ? "hinhthuclamviecs.aspx" : strLink;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DeleteInfo()
        {
            try
            {
                var G_info = DB.GetTable<VL_HINHTHUCLAMVIEC>().Where(g => g.ID == _id);

                DB.VL_HINHTHUCLAMVIECs.DeleteAllOnSubmit(G_info);
                DB.SubmitChanges();
                Response.Redirect("hinhthuclamviecs.aspx");
            }
            catch (Exception ex)
            {
                clsVproErrorHandler.HandlerError(ex);
        ...
        {
            return "hinhthuclamviec.aspx?id=" + Utils.CStrDef(GroupId);
        }
